...
class ListPRsReviewing extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            shouldHideAlreadyReviewed:
                props.checkboxes.shouldHideAlreadyReviewed,
    ...
    toggleDraftFilter() {
        this.setState(prevState => ({
            shouldHideDraft: !prevState.shouldHideDraft,
        }));
        PullRequests.saveCheckboxes({
            shouldHideDraft: !this.state.shouldHideDraft,
        });
    }

    toggleAlreadyReviewedFilter() {
        this.setState(prevState => ({
            shouldHideAlreadyReviewed: !prevState.shouldHideAlreadyReviewed,
        }));
        PullRequests.saveCheckboxes({
            shouldHideAlreadyReviewed: !this.state.shouldHideAlreadyReviewed,
        });
    }

    toggleOnHoldFilter() {
        this.setState(prevState => ({
            shouldHideOnHold: !prevState.shouldHideOnHold,
        }));
        PullRequests.saveCheckboxes({
            shouldHideOnHold: !this.state.shouldHideOnHold,
        });
    }
    ...
    filterPRs(prs) {
        const terms = _.filter(
            this.state.searchText.trim().split(/\s+/),
            term => term.length > 0,
        );
        const includeTerms = [];
        const excludeTerms = [];
        ...
        return _.filter(prs, (pr) => {
            ...
            if (
                this.state.shouldHideAlreadyReviewed
                && pr.isReviewRequested === false
            ) {
                return false;
            }
            if (
                this.state.shouldHideOnHold
                && (pr.title.indexOf('[HOLD') > -1
    ...
    render() {
        ...
        const filteredPRs = this.props.prs
            ? this.filterPRs(this.props.prs)
            : [];
