    ...
    return (
      <li className="Box-row">
        <div className="pull-right actionbuttons">
          <button className={`btn tooltipped tooltipped-sw ${this.state.showCommentBar ? 'selected' : ''}`} aria-label="Add comment" onClick={this.toggleCommentBar}>💬</button>
          {' '}
          {this.props.data.assignee && this.isOverdue && <button className="btn tooltipped tooltipped-sw" aria-label="Send a quick comment to bump the assignee for an update" onClick={this.postQuickBump}>{this.state.quickBumpText}</button>}
          {' '}
