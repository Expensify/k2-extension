...
module.exports = React.createClass({
  ...
  componentWillMount() {
    ...
    this.isUnderReview = _(this.props.data.labels).find(l => l.name.toLowerCase() === 'reviewing');
    this.isOverdue = _(this.props.data.labels).find(l => l.name.toLowerCase() === 'overdue');
      ...
      <li className="Box-row">
        <div className="pull-right actionbuttons">
          <BtnGroup>
            <button
              ...
              onClick={this.toggleCommentBar}>💬</button>
            <button
            ...
            <button
              ...
              onClick={this.props.onMoveDown}>▼</button>
            {!this.state.showConfirmKill && this.isClosed && this.isOnStagingAndProduction && <button
              className="btn tooltipped tooltipped-sw"
              aria-label="Remove issue from What's Next"
              onClick={this.toggleConfirmKill}>☢</button>}
            {this.state.showConfirmKill && <button
              className="btn"
              onClick={this.removeIssue}>U sure?</button>}
            {this.state.showConfirmKill && <button
              className="btn"
              onClick={this.toggleConfirmKill}>Cancel</button>}
            {this.props.data.assignee && this.isOverdue && <button
              className="btn tooltipped tooltipped-sw"
              aria-label="Send a quick comment to bump the assignee for an update"
              onClick={this.postQuickBump}>{this.state.quickBumpText}</button>}
          </BtnGroup>
        </div>
        ...
        {this.state.lastComment && (
          <div className={`timeline-comment-wrapper js-comment-container lastcomment ${this.isOnHold ? 'hold' : ''}`}>
            <div dangerouslySetInnerHTML={{__html: `<em class="pull-left">(${lastCommentDate}):&nbsp;</em> ${this.state.lastComment.body_html}`}} />
          </div>
        )}

        {!this.isOnHold && this.state.showCommentBar && <div className="form-inline">
