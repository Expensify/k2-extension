        ...
        _.each(data.items, function(item) {
          ...
          $.ajax({
            ...
            $.ajax({
              ...
              // If the statuses URL returns [], it is either because there are
              // no statuses or because the repo is using the new GitHub App as opposed
              // to "an older Travis CI OAuth App". The newer GitHub App uses
              // the check-runs API endpoint instead of statuses, so try that one.
              if (data3.length === 0) {
                $.ajax({
                ...
                }).done(function(crData) {
                  // Check-runs endpoint uses .conclusion instead of .state - map it
                  // back to .state in order not to change the view
                  item.pr.status = (crData.check_runs || []).map(status => Object.assign({state: status.conclusion}, status));
                });
              } else {
