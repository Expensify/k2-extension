...
function getOrganizationMembers(cb) {
  ...
  function handleMemberGraphQLData(data, status, xhr) {
    console.log(arguments);
    // Only get our open milestones
    result = result.concat(data.data.organization.members.nodes);

    // If we have a next page, then let's fetch it
    if (data.data.organization.members.pageInfo.hasNextPage) {
      /* eslint-disable no-use-before-define */
      recursiveMemberQuery(data.data.organization.members.pageInfo.endCursor);
      /* eslint-enable no-use-before-define */
      return;
    }
