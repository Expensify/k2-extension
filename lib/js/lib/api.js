    ...
    function handleData(data, status, xhr) {
      ...
      const sortedData = _.chain(data)
        .each(i => {
          ...
          i.score = score;
          i.percentComplete = i.open_issues || i.closed_issues ? Math.round((i.closed_issues / (i.open_issues + i.closed_issues)) * 100) : 0;
        })
        .sortBy('score')
        .value();
 ...
 * @param {Function} cb
 */
function getIssuesForMilestone(type, milestone, cb) {
  let query = '?q=';
  let url;
