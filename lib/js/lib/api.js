...
function getMilestones(cb) {
  ...
  prefs.get('whatsnextview', whatsnextview => {
    ...
    GistDB.get(`${currentView}.hidden`, (err, gistData) => {
      if (err) {
        return;
      }
      ...
      function handleData(data, status, xhr) {
        ...
        const sortedData = _.chain(data)
          .each(i => {
            i.hidden = _(gistData).contains(i.id);

            // Start the score at 0 if they are hidden so that they are at the bottom
            let score = i.hidden ? 0 : 100000;

            i.score = score;
            i.percentComplete = Math.round((i.closed_issues / (i.open_issues + i.closed_issues)) * 100);
          })
          .sortBy('score')
          .value();
  ...
  // TODO: GraphQL still isn't full featured enough to use (not enough data or functionality), but this code works.
  // let result = [];
  // const currentUser = $('.user-nav .avatar').attr('alt').replace('@', '');
  // const url = 'https://api.github.com/graphql';

  // function handleMilestoneGraphQLData(data, status, xhr) {
  //   // Only get our open milestones
  //   result = result.concat(_(data.data.organization.repository.milestones.nodes).where({state: 'OPEN'}));

  //   // If we have a next page, then let's fetch it
  //   if (data.data.organization.repository.milestones.pageInfo.hasNextPage) {
  //     /* eslint-disable no-use-before-define */
  //     recursiveMilestoneGraphQL(data.data.organization.repository.milestones.pageInfo.endCursor);
  //     /* eslint-enable no-use-before-define */
  //     return;
  //   }
  ...
  // function recursiveMilestoneGraphQL(cursor) {
  ...
  //       })
  //     })
  //       .done(handleMilestoneGraphQLData)
  //       .fail(function(err) {
  //         cb(err);
  //       });
  //   });
...
function getPullsReviewing(cb) {
  ...
  const done = _.after(2, () => {
    cb(null, _(result).chain()
      .filter(pr => {
        return pr.assignee && pr.assignee.login !== getCurrentUser();
      })
...
/**
 * Get all the bugs that are not assigned and are dailys
 *
 * @author Tim Golen <tim@golen.net>
 *
 * @date 2015-06-07
...
exports.getDailyBugs = getDailyBugs;
