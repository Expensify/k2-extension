...
module.exports = React.createClass({
  ...
  callApi() {
    ...
    API[this.props.apiMethod]((error, data) => {
      ...
      $(`[data-key="${this.props.apiMethod}"] .Counter`).html(data.length > 99 ? '100+' : data.length);
  ...
  filterData(filters) {
    return _(this.unfilteredData).filter(item => {
      const isImprovement = _(item.labels).findWhere({name: 'Improvement'});
      const isTask = _(item.labels).findWhere({name: 'Task'});
      const isFeature = _(item.labels).findWhere({name: 'NewFeature'});
